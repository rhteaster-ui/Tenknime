...
// --- CONFIG MODIFIED FOR TENKZ OFC ---
// Color: 'orange' (Primary) & 'gold' (Accent) sesuai style.css baru
const HOME_SECTIONS = [
    { title: "LayarOtaku Update", type: "latest", badge: "Ongoing", color: "orange" },
    { title: "Trending Action", type: "query", queries: ["jujutsu", "solo leveling", "kaiju", "wind breaker"], badge: "Hot", color: "gold" },
    { title: "Donghua Server", type: "query", queries: ["donghua", "btth", "soul land", "swallowed"], badge: "Anichin", color: "orange" },
    { title: "Romance Picks", type: "query", queries: ["romance", "harem", "school", "roshidere"], badge: "Love", color: "gold" }
];
...
async function handleSearch(manualQuery = null, isListTab = false) {
    ...
    container.innerHTML = `<div class="section-title"><div class="bar-accent gold"></div><h3>Hasil: "${query}"</h3></div>`;
...
function renderHistorySection() {
    ...
    container.innerHTML = history.map(h => `
        <div class="anime-card" onclick="resumeVideo('${h.epUrl}', '${h.epNum}', '${h.mainUrl}', '${h.title}', '${h.image}', ${h.seconds})">
            <span class="badge-ep" style="background: linear-gradient(45deg, #ff6b01, #ffa502); color: #000; font-size:0.75rem;">Lanjut Ep ${h.epNum || '?'}</span>
